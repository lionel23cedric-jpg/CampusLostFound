...
| ADMIN-05 | Configure Staff membership | Administrator | `account-role-service.test.ts`, `admin-account-role-route.test.ts`, `admin-account-management-client.test.tsx` | Promote a registered test student, sign in again, then demote | Role change is audited, sessions are revoked, and administrator promotion is unavailable | Automated + browser pass |
...
record what was demonstrated. `npm audit` reports zero known
...
database on 16 September 2026. Live Staff promotion/demotion and a browser
AI-assisted label demonstration passed on disposable records on 22 September
2026. Wider responsive, accessibility, and all-page visual checks remain open.
